...
// trk_0/p  : fit rpc07b0
//-----------------------------------------------------------------------------
plot_data_t* plot_figure_0071(int Figure, int Print) {
    
  const char* rpc07b0s51 = "pipenu.rpc07b0s51r0200";
  //  const char* cele2b0s51 = "pipenu.cele2b0s51r0100";

  const char* book       = "pipenu";
  const char* ana_job    = "kate_trk_ana.0000";
  const char* ana_module = "Stntuple_TrackAna";
  const char* hist_name  = "trk_5/eetrkmom";
...
// 
//------------------------------------------------------------------------------
  p.hd[0]              = hist_data_t(catalog,book,rpc07b0s51,ana_job,ana_module,hist_name);
  ...
  cbx->init_parameters(120,128.64,0.158,11.2,5,2,1.3,2.);
  cbx->fit(120,135);
