...
\add{The peak at 129.4 MeV corresponds to photons produced in the $\pi^- p \to n \gamma$ reaction,
...
\del{
  Events of interest for calibration are the monochomatic RPC photons at 129.4 MeV
  from $\pi^{-} p \to n \gamma$ on hydrogen. Measurement of this process is shown in 
...
\add{The 129.4 MeV $\pi^{-} p \to n \gamma$ peak is also clearly seen on hydrogen-containing compounds.
...
\del{For the $\rm{CH_2}$ spectrum in 
Figure~\ref{figure:1971_bistirlich_fig_09_ch2}
, photons from RPC on carbon occupy the region between the two hydrogen peaks.
}
To produce a \del{photon in the 129.4 MeV peak}\add{129.4 MeV photon},
\add{(i)} a negative pion stopped on a $\rm{CH_2}$  degrader must
\del{both (i)} \add{be captured}\del{capture} on a hydrogen rather
than carbon nucleus, and (ii) \add{the capture must be followed by a $\pi^{-} p \to n \gamma$ reaction}\del{interact radiatively with the proton via $\pi^{-} p \to n \gamma$ rather than producing an intermediate $\pi^0$}.
